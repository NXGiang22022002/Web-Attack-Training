...
<div class="card">
  <form method="POST" action="{{ route('teacher.students.store') }}">
    ...
    <input name="tendangnhap" value="{{ old('tendangnhap') }}" required>

    <label>Mật khẩu</label>
    <input type="password" name="matkhau" required>

    <label>Họ tên</label>
    <input name="hoten" value="{{ old('hoten') }}" required>

    <label>Email</label>
    <input name="email" value="{{ old('email') }}">

    <label>Số điện thoại</label>
    <input name="sodienthoai" value="{{ old('sodienthoai') }}">

    <div style="margin-top:14px;display:flex;gap:10px;">
      <button class="btn btn-primary" type="submit">Lưu</button>
      <a class="btn btn-w" href="{{ route('teacher.students.index') }}">Hủy</a>
    </div>
  </form>
</div>
